...
3. Run two isolated reviews in parallel, both through the `senior-review` dispatch contract with the same inputs: resolved paths, base SHA, head SHA, intended local changes, objective and accepted trade-offs. The first is `senior-review` itself (correctness, evidence-based). The second is the thermo-nuclear code quality review (structure, abstractions, spaghetti growth, file size): hand that child `$HOME/.agents/skills/thermo-nuclear-code-quality-review/SKILL.md` as its reviewer instructions in place of `references/reviewer.md`, and ask for the same compact return. That skill is user-invocation only, so it is never called through the skill tool; the child reads the file. If the file is absent, say so and continue with the senior review alone. Both reviews cover the complete proposed PR change, including relevant uncommitted and untracked content. Do not load the reviewer instructions or full reports into the parent by default. If isolation is unavailable, report that limit rather than silently claiming an independent review.
4. Verify findings from both reviews in source, de-duplicated. Fix concrete problems within the task, retain evidence for rejected findings, and leave optional redesigns optional. A structural finding from the quality review is fixed when it fits the task and preserves behavior; a wider restructuring goes into the approval summary as a proposed follow-up, never silently into this lot. Recheck changed paths and affected callers after fixes; reuse the reviewer for a focused follow-up when needed. A changed snapshot is not automatically covered by an earlier review.
...
Present a compact approval summary: final behavior, included files or hunks and exclusions, existing outgoing commits, proposed commit split/messages, both review verdicts and unresolved findings, checks, destination branch/remote and PR base. Give enough evidence to assess the completed result without generating a patch handoff. Ask for approval of commit + push + PR publication and in-scope review replies/resolutions if that authorization is not already explicit. Explain that this is the user's chosen control point in this skill.
